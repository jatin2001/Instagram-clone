                    ...
                    : data.map((item) => {
                        return (
                            <div key={item._id} className="card home-card">
                                ...
                                <h5><Link to={(item.postedBy._id === state._id) ? '/profile' : `/user/${item.postedBy._id}`}>{item.postedBy.name}</Link> {(item.postedBy._id === state._id) && <i className="material-icons" style={{ float: "right" }} onClick={() => deletePost(item._id)}>delete</i>}</h5>
                                ...
                                <div className="card-content">
                                    <i className="material-icons" style={{ color: "red" }} onClick={() => {
                                        ...
                                        {item.likes.includes(state._id) ? "favorite" : "favorite_border"}
                                    </i>
                                                ...
                                                <div key={cmt._id} style={{ display: "flex", justifyContent: "space-between" }}>
                                                    <div>
                                                        <p style={{ fontSize: "16px" }}>
                                                            <strong>{cmt.postedBy.name} </strong>
                                                            {cmt.text}
                                                        </p>
                                                    </div>
                                                    {
